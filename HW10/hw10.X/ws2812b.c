...
#define LOWTIME 15//17 // number of 48MHz cycles to be low for 0.35uS
#define HIGHTIME 65//79 // number of 48MHz cycles to be high for 1.65uS
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].r & 1<<j)==1<<j)/* identify the bit in c[].r, is it 1 */ {
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].g & 1<<j)==1<<j)/* identify the bit in c[].r, is it 1 */ {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].b & 1<<j)==1<<j)/* identify the bit in c[].r, is it 1 */ {
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
